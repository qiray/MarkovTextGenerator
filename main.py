...
def get_random_token(data, count):
...
def get_next_token_for_start(start):
    data, count = database.get_pairs_for_start(start)
    return get_random_token(data, count)
...
def generate_sequence(number):
    ...
    if not start:
        return ''
    next_token = get_next_token_for_start(start) #get all pairs for selected start
    tokens_list = [start, next_token]
    while not database.is_token_end(next_token): #while chosen pair is not end
        next_token = get_next_token_for_start(next_token) # get another list of pairs
...
def parse_args():
    ...
    parser.add_argument("-n", "--number", help='Set size of token for text parsing (default = {})'.format(N), type=int)
...
def main():
    ...
    if args.generate:
        sentence = generate_sequence(number)
        print(sentence)
    if args.version:
